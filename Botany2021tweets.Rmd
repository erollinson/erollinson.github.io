...
dt_tweets <- read.csv("Botany2021FullWeek.csv", stringsAsFactors = FALSE)
...
Tweets using the hashtag #Botany2021 were aggregated from Twitter using the R package [rtweet](https://cran.r-project.org/web/packages/rtweet/index.html) and the Twitter API. The summary statistics are static as of 09:00 AM EDT 7/24/2021 (additional tweets, retweets, and likes after that point are not included in the summary, although embedded tweets are "live" and up-to-date).
...
ggplot(top_orig_users) + geom_bar(aes(x = screenName, y = Original), stat = "identity", fill=pal[7]) +
  ...
  coord_flip() +
    theme_bw() +
    labs(title="Most #Botany2021 Tweets (excluding retweets)", subtitle = "Sun - Fri, Top 50 Users") +
...
ggplot(top_users) + 
  ...
  theme_bw() +
  labs(title="Most #Botany2021 Tweets (including retweets)", subtitle = "Sun - Fri, Top 50 Users") +
  theme(axis.text = element_text(size = 11, family = "Open Sans", color="black"),
        legend.text = element_text(size = 12, family="Open Sans"),
        legend.title = element_blank(),
        axis.title.x = element_text(size = 12, family = "Open Sans"),
...
## 🐈
Quite a few cats attended #Botany2021 - [click here for the cat parade](https://erollinson.github.io/Botany2021Cats)
