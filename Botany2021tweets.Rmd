...
# save_as_csv(tweets, "Botany2021Tweets_7_11.csv")
...
Tweets using the hashtag #Botany2021 were aggregated from Twitter using the R package [rtweet](https://cran.r-project.org/web/packages/rtweet/index.html) and the Twitter API. The summary statistics are static as of 8:00 AM EDT Aug 01 2020 (additional tweets, retweets, and likes after that point are not included).
...
htmltools::tagList(lapply(b$status_url, tweet_embed, plain=FALSE))
...
htmltools::tagList(lapply(c$status_url, tweet_embed, plain=FALSE))
```
