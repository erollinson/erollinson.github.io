...
```{r} 
#load flora
species <- read.csv("Plantsgiving/plantsgiving2020.csv")
tree <- read.tree("Plantsgiving/plantsgivingtree2020.new") #this is currently importing the non branch length adjusted tree 
```
...
```{r}
species2 <- species %>%
  select(Common, Family, SpPhylo)
str(species2)
species2$SpPhylo <- as.character(species2$SpPhylo)
species2$Common <- as.character(species2$Common)
```
...
```{r}
tree2<-tree
tree2$tip.label<- species2$Common[match(tree2$tip.label, species2$SpPhylo)]
...
```{r}
#circular tree
round<- ggtree(tree2, layout="circular") %<+% species2 + geom_tiplab2(size=3, aes(angle=angle, color=Family)) + scale_color_viridis_d(option="viridis") + theme(legend.position="right")
round
...
Repeating the process above with a second data file compiling species across participants.

```{r}
#load flora
species <- read.csv("Plantsgiving/allspecies2020.csv")
tree <- read.tree("Plantsgiving/AllSpecies2020.new") #this is currently importing the non branch length adjusted tree
...
#circular tree no color
round<- ggtree(tree2, layout="circular") %<+% species2 + geom_tiplab2(size=3, aes(angle=angle)) + theme(legend.position="right")
round
```