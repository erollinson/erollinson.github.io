...
#library(remotes) #run once to import fonts; code below assumes Open Sans installed locally
#remotes::install_version("Rttf2pt1", version = "1.3.8") #run once to import fonts; code below assumes Open Sans installed locally
library(extrafont)
#font_import() #run once to import fonts; code below assumes Open Sans installed locally
#extrafont::loadfonts(device="win") #run once to import fonts; code below assumes Open Sans installed locally #deprecated
library(dplyr)
library(ggplot2)
library(knitr)
library(tidyr)
...
```{r, fig.height=10, echo=FALSE, warning=FALSE, fig.alt = "Bar plot showing the number of different accounts retweeted by each user, within tweets using the #Botany2022 hashtag between Sunday July 24 and Wednesday July 27, 2022. Each bar shows the number of accounts that a particular user retweeted within this hashtag, for the 50 users that retweeted the most other accounts. For instance, SarraceniaMason retweeted tweets using the #Botany2022 hashtag from 120 other accounts. The values in this figure are also included in the plain text tables lower on this page."}
#Suggestions -  "Next year it would be cool to see rankings built around building community on twitter.  I.e., who are the BigName accounts RT-ing smaller accounts to help them build connections? Who are the accounts deliberately seeking out underrepresented folx, newbies, students, etc?"  - Az Klymiuk
...
ggplot(smallplot) + 
  ...
  coord_flip() +
  theme_bw() +
  labs(title="Number of Smaller Accounts Retweeted", subtitle = "June 24 - 27, all users that retweeted >1 account with <250 followers") +
  theme(axis.text = element_text(size = 11, family = "Open Sans", color="black"),
        legend.text = element_text(size = 12, family="Open Sans"),
        legend.title = element_blank(),
        axis.title.x = element_text(size = 12, family = "Open Sans"),
...
```{r, fig.height=10, echo=FALSE, warning=FALSE, fig.alt = "Bar plot showing the number of times users tagged smaller accounts (within tweets and accounts using the #Botany2022 hashtag between Sunday July 24 and Wednesday July 27, 2022). 'Smaller' accounts is defined here as accounts with fewer than 250 followers; this is roughly one quarter of all accounts using the hashtag during the conference. The values in this figure are also included in the plain text tables lower on this page."}
...
ggplot(small_tags) + 
  ...
  ylab("Number of mentions of an account with <250 followers") +
  coord_flip() +
  theme_bw() +
  labs(title="Number of mentions of a small account", subtitle = "June 24 - 27, all users that mentioned at least 1 small account") +
...
### Number of tags of small accounts
```{r echo = FALSE, warning = FALSE}
table6 <- small_tags %>%
